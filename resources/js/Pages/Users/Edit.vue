    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex justify-between items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <div>
            <button v-if="!user.deleted_at && actions.includes('delete')" class="btn-warning ml-aut" tabindex="-1" type="button" @click="destroy">Delete User</button>
          </div>
          <div class="flex items-end space-x-1">
            <Link :href="'/users'" class="btn-secondary ml-auto"> Back </Link>
            <loading-button v-if="actions.includes('update')" :loading="form.processing" class="btn-indigo ml-auto" type="submit">Update User</loading-button>
          </div>
        </div>
      </form>
    </div>
